(() => {
  ...
  function photoMarkup(size=44){const cfg=getConfig(),photo=cfg.profilePhoto||'';if(photo)return `<img src="${photo}" alt="Foto de perfil" style="width:${size}px;height:${size}px;border-radius:50%;object-fit:cover;display:block;border:3px solid #fff;box-shadow:0 0 0 1px #dbe3ee;background:#fff">`;return `<div style="width:${size}px;height:${size}px;border-radius:50%;display:grid;place-items:center;background:#eaf2ff;color:#2563eb;font-weight:900;font-size:${Math.max(12,Math.round(size*.32))}px;border:3px solid #fff;box-shadow:0 0 0 1px #dbe3ee">${initials()}</div>`}
  function renderSidebarPhoto(force=false){
    ...
    bar?.querySelector('.profile-photo-sidebar')?.remove();
    let box=sidebar.querySelector('.profile-photo-top');
    if(!box){box=document.createElement('div');box.className='profile-photo-top';brand.insertAdjacentElement('afterend',box)}
    const cfg=getConfig(),signature=(cfg.profilePhoto||'')+'|'+initials();
    if(force||signature!==lastSidebarSignature){
      box.innerHTML=`<div class="profile-photo-top-inner">${photoMarkup(76)}<div class="profile-photo-top-text"><strong>${(cfg.nomeUsuario||'Gabriel').trim()||'Gabriel'}</strong><span>${(document.getElementById('contaEmail')?.textContent||'').trim()}</span></div></div>`;
  ...
  async function handlePhoto(e){const file=e.target.files?.[0];if(!file)return;const status=document.getElementById('profilePhotoStatus');if(status)status.textContent='Processando foto...';try{if(!/^image\/(jpeg|png|webp)$/i.test(file.type))throw new Error('Use uma imagem JPG, PNG ou WEBP.');if(file.size>12*1024*1024)throw new Error('A imagem é muito grande. Escolha uma foto de até 12 MB.');const data=await compressImage(file),cfg=getConfig();cfg.profilePhoto=data;saveConfig(cfg);renderSidebarPhoto(true);renderSettingsPhoto();if(status)status.textContent='✅ Foto salva e sincronizada.'}catch(err){if(status)status.textContent='❌ '+(err.message||'Não foi possível salvar a foto.')}e.target.value=''}
  function removePhoto(){const cfg=getConfig();delete cfg.profilePhoto;saveConfig(cfg);renderSidebarPhoto(true);renderSettingsPhoto();const s=document.getElementById('profilePhotoStatus');if(s)s.textContent='Foto removida.'}
  function preservePhotoAcrossConfigSave(){if(typeof window.salvarConfiguracoes!=='function'||window.salvarConfiguracoes.__profileWrapped)return;const original=window.salvarConfiguracoes;const wrapped=function(){const before=getConfig(),photo=before.profilePhoto||'',out=original.apply(this,arguments);if(photo){const after=getConfig();if(!after.profilePhoto){after.profilePhoto=photo;saveConfig(after)}}renderSidebarPhoto(true);renderSettingsPhoto();return out};wrapped.__profileWrapped=true;window.salvarConfiguracoes=wrapped}
  const style=document.createElement('style');style.textContent=`.profile-photo-top{padding:14px 18px 16px;border-bottom:1px solid #e5eaf1}.profile-photo-top-inner{display:flex;align-items:center;gap:12px}.profile-photo-top-text{min-width:0;display:flex;flex-direction:column;gap:3px}.profile-photo-top-text strong{font-size:14px;color:#172033;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.profile-photo-top-text span{font-size:10px;color:#64748b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.conta-barra .profile-photo-sidebar{display:none!important}@media(max-width:760px){.profile-photo-top{padding:10px 14px}.profile-photo-top-inner img,.profile-photo-top-inner>div:first-child{width:54px!important;height:54px!important}}`;document.head.appendChild(style);
  function refresh(){renameHome();preservePhotoAcrossConfigSave();ensureSettingsCard();renderSidebarPhoto();renderSettingsPhoto()}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(refresh,0));else setTimeout(refresh,0);window.addEventListener('storage',e=>{if(e.key===CONFIG_KEY)refresh()});window.addEventListener('focus',refresh);
})();
